...
void visualize_path()
{
  std::string command = "roslaunch sparseRRT disp_obst_traj.launch map:=" + params::map
			+ " file_name:=trajectory_rrt_search.csv";
...
public:
  ...
  tree(std::string path_file, std::string map, int iter_time, double start[3], double goal[3])
  {
    ...
    rrt_search = n.advertiseService("rrt_tree_search",&tree::search_nodes_in_tree, this);
    tree_built = false;    
  ...
  bool search_nodes_in_tree(sparseRRT::rrt_search::Request &req,
                            sparseRRT::rrt_search::Response &res)
  {
    //space map limits
    ...
    //ROS_INFO_STREAM("Everithing okey baby");
  ...
  */
  ...
  void create_tree()
  {
    ...
    tree_built = true;
    n.setParam("available_rrt_service", 1);
    infinite_loop();
  }
...
{
  ros::init(argc, argv, "quadrotor_RRT_tree_service");
  ...
  else
  {
    ...
    //fill start position
    rrt_start[0] = atof(argv[4]);
    rrt_start[1] = atof(argv[5]);
    rrt_start[2] = atof(argv[6]);

    //and goal position
    rrt_goal[0] = atof(argv[7]);
    rrt_goal[1] = atof(argv[8]);
    rrt_goal[2] = atof(argv[9]);
  ...
  tree *t = new tree(path_file, map_name, iter_time, rrt_start, rrt_goal);
  ...
  if(start_in_obstacle || goal_in_obstacle)
  {
    ...
    n.setParam("available_rrt_service", 2);
  }
  else
  {
