...
{
  ros::init(argc, argv, "quadrotor_SST");
  ...
  else
  {
    ...
    sparse_goal[0] = atof(argv[4]);
    sparse_goal[1] = atof(argv[5]);
    sparse_goal[2] = atof(argv[6]);

    sparse_start[0] = atof(argv[7]);
    sparse_start[1] = atof(argv[8]);
    sparse_start[2] = atof(argv[9]);
  ...
  while(ros::ok() && !path_done){
	...
	params::path_file = argv[1];                
        ...
        params::goal_state[0] = sparse_goal[0];
        params::goal_state[1] = sparse_goal[1];
        params::goal_state[2] = sparse_goal[2];
        
        params::start_state[0] = sparse_start[0];
        params::start_state[1] = sparse_start[1];
        params::start_state[2] = sparse_start[2];
	...
	if (params::map == "Map5_3D")
	{
	  system->change_Map_Limits(5.0, 6.0, 0.5, 4.0);
        ...
        if (goal_in_obstacle)
        {
          ...
          n.setParam("available_sst_traj_scratch", 2);
	...
	if (params::availableSolution == true)
        {
          ...
          //std::cout<<"Visualize results in RVIZ" << std::endl;
	  visualize_path();
          n.setParam("available_sst_traj_scratch", 1);
	}
	else
	{
