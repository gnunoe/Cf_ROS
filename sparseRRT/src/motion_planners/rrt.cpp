...
bool rrt_t::path_between_nodes(double x_s, double y_s, double z_s,
                               ...
                               bool smooth)
{
        ...
        system->copy_state_point(metric_query->point,sample_state);
        int num_start_neighbors = metric->find_delta_close_and_closest(metric_query,close_nodes,distances,0.2);
        ...
        system->copy_state_point(metric_query->point,sample_state);
        int num_goal_neighbors = metric->find_delta_close_and_closest(metric_query,close_nodes,distances,0.2);
        ...
        //DONE:Crash when you supply the root point
...
void rrt_t::smooth_path(std::deque<tree_node_t*> &start_path, std::deque<tree_node_t*> &goal_path, std::vector<double> &sg,double old_cost)
{
        ...
        if (s_size<g_size)
        {
                ...
                less_cost_element = search_points_to_smooth(start_path, goal_path, neighbor, old_cost);
        ...
        else
        {
                ...
                less_cost_element = search_points_to_smooth(goal_path, start_path, neighbor, old_cost);
                if (less_cost_element == -1)
                {
                        ROS_INFO_STREAM("Impossible to smooth the trajectory");
                        compose_trajectory(start_path, goal_path, sg, false, neighbor, false, false);
...
int rrt_t::search_points_to_smooth(std::deque<tree_node_t*> &small_segment, std::deque<tree_node_t*> &big_segment,
                                   std::vector <tree_node_t*> &neighbor, double old_cost)
{
  	...
  	if (true)
        ...
        for (int i=small_segment.size()-1; i>small_segment.size()-small_size; i--)
        {
                ...
                int num_neighbors_1 = metric->find_delta_close_and_closest(metric_query,close_nodes,distances,0.2);
		...
		for (int j=0; j<num_neighbors_1; j++)
                {
                        ...
                        for (int k=big_segment.size()-1; k>big_segment.size()-big_size; k--)
                        {
                                ...
                                int num_neighbors_2 = metric->find_delta_close_and_closest(metric_query,close_nodes,distances,0.2);
...
void rrt_t::compose_trajectory(const std::deque<tree_node_t*> &start_traj, const std::deque<tree_node_t*> &goal_traj, std::vector<double> &sg, 
			       bool smooth, const std::vector<tree_node_t*> &shared_neighbor, bool start_is_root, bool goal_is_root)
{
	...
	//End point
        ...
        //Write path in file
        std::string dir = params::path_file + "/trajectory_rrt_search.csv";
...
void rrt_t::get_parent(int jumps, tree_node_t * node)
{
	tree_node_t * n = node;
...
bool rrt_t::search_shared_node(int num_close_neighbors, std::deque<tree_node_t*> &close_neighbors, std::deque<tree_node_t*> &close_to_shared, 
