...
private:
	...
	static void* threadWrapper(void* arg) {
		...
		ROS_INFO_STREAM("Thread Wrapper");
		return NULL;
	}
	...
	void executeTrajectory(){
		if(toExecute.joint_names[0]=="virtual_joint" && toExecute.points.size()>0){
			...
			for(int k=0; k<toExecute.points.size(); k++){
				//ROS_INFO_STREAM(toExecute.points[k].velocities[0].linear.x);		
		
				//ricavo cmd da effettuare
				geometry_msgs::Transform_<std::allocator<void> > punto=toExecute.points[k].transforms[0];
                                ROS_INFO_STREAM(punto.translation.x <<" "<< punto.translation.y <<" "<< punto.translation.z);
				//ROS_INFO_STREAM(punto.velocities.x <<" "<< punto.velocities.y <<" "<< punto.velocities.z);
				bool eseguito=true;
				if(k!=0){
